...
<body class="p-4">
        ...
        <form class="border p-4 bg-light" action="{{ route('reviews.update', $review->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                @if($review['status'] == 'pending')
                    ...
                    <button type="button" class="btn btn-danger mt-3" data-toggle="modal" data-target="#declineModal">Decline</button>
                @else
                    <input type="text" id="status" name="status" value="{{ $review['status'] }}" class="form-control" readonly>
                @endif
    ...
    <div class="modal fade" id="declineModal" tabindex="-1" role="dialog" aria-labelledby="declineModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="declineModalLabel">Decline Review</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    <script>
        ...
        document.getElementById('status_action').value = 'declined';
        document.querySelector('.container-sm form').submit();
    }
    </script>
</body>
